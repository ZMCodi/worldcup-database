...
echo "$($PSQL "SELECT w.name FROM games g LEFT JOIN teams w ON g.winner_id = w.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION ALL SELECT o.name FROM games g LEFT JOIN teams o ON g.opponent_id = o.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name;")"
